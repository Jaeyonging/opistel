...
const OpistelContainer = ({ currCityNumber, selectedYear, selectedMonth, searchQuery }: Props) => {
    ...
    return (
        <>
            <ApiErrorBoundary>
                <FetchOpistelData code={currCityNumber} date={formattedDate}>
                    <div ref={listRef} className='flex flex-col gap-2'>
                        {paginatedData.map((item: any, index: number) => (
                            <OpistelCard key={`${item?.rletNo || index}-${index}`} data={item} />
                        ))}
                    </div>

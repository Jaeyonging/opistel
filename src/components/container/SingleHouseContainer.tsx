...
const SingleHouseContainer = ({ currCityNumber, selectedYear, selectedMonth, searchQuery }: Props) => {
    ...
    return (
        <>
            <ApiErrorBoundary>
                <FetchSingleHouseData code={currCityNumber} date={formattedDate}>
                    <div ref={listRef} className='flex flex-col gap-2'>
                        {paginatedData.map((item: any, index: number) => (
                            <SingleHouseCard key={`${item?.houseType || index}-${index}`} data={item} />
                        ))}
                    </div>
